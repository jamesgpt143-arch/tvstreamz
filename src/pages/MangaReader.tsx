...
const MangaReader = () => {
  ...
  const nextPage = useCallback(() => {
    if (currentPage < totalPages - 1) {
      ...
      if (currentIndex < chapters.length - 1) {
        navigate(`/manga/${mangaId}/read/${chapters[currentIndex + 1].id}`);
      }
    }
  }, [currentPage, totalPages, chapters, chapterId, mangaId, navigate]);
  ...
  }, [currentPage, chapters, chapterId, mangaId, navigate]);
        ...
        <div className="container mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              className="text-white hover:bg-white/10"
              onClick={() => setImageQuality(imageQuality === 'data' ? 'data-saver' : 'data')}
              title={imageQuality === 'data' ? 'Using HD Quality' : 'Using Data Saver'}
            >
              <Settings className={`w-5 h-5 ${imageQuality === 'data-saver' ? 'text-yellow-400' : ''}`} />
            </Button>
      ...
      {readingMode === 'page' ? (
        ...
        <div className="min-h-screen flex items-center justify-center p-4 pt-20 pb-24">
          <img
            src={getChapterPageUrl(
              pages.baseUrl,
              pages.hash,
              imageQuality === 'data' ? pages.data[currentPage] : pages.dataSaver[currentPage],
              imageQuality
            )}
        ...
        <div className="pt-20 pb-24">
          <div className="max-w-3xl mx-auto">
            {(imageQuality === 'data' ? pages.data : pages.dataSaver).map((filename, index) => (
              <img
                key={index}
                src={getChapterPageUrl(pages.baseUrl, pages.hash, filename, imageQuality)}
                alt={`Page ${index + 1}`}
                className="w-full"
                loading="lazy"
              />
        ...
        <div className="container mx-auto">
          ...
          <div className="flex items-center justify-between">
            <Button
              ...
              onClick={() => hasPrevChapter && goToChapter(chapters[currentChapterIndex - 1].id)}
              className="text-white hover:bg-white/10"
            >
            ...
            <Button
              ...
              onClick={() => hasNextChapter && goToChapter(chapters[currentChapterIndex + 1].id)}
              className="text-white hover:bg-white/10"
            >
              Next Chapter
